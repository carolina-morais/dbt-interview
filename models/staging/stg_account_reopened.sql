...
{{
  config(
    ...
    partition_by = {'field': 'reopened_ts', 'data_type': 'timestamp'},
...
select 
  {{ dbt_utils.generate_surrogate_key(['account_id_hashed', 'reopened_ts']) }} as surrogate_key
  , reopened_ts
  , account_id_hashed
  , {{add_metadata()}}                                                 
...
{% if is_incremental() %}
where reopened_ts in ({{ partitions_to_replace | join(',') }})
{% endif %}
